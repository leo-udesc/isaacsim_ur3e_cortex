...
//
#ifndef RANGESENSORSCHEMA_GENERATED_LIDAR_H
#define RANGESENSORSCHEMA_GENERATED_LIDAR_H

/// \file rangeSensorSchema/lidar.h
...
/// \class RangeSensorLidar
///
///
class RangeSensorLidar : public RangeSensorRangeSensor
{
public:
    ...
    /// Construct a RangeSensorLidar on UsdPrim \p prim .
    /// Equivalent to RangeSensorLidar::Get(prim.GetStage(), prim.GetPath())
    /// for a \em valid \p prim, but will not immediately throw an error for
    /// an invalid \p prim
    explicit RangeSensorLidar(const UsdPrim& prim=UsdPrim())
        : RangeSensorRangeSensor(prim)
    {
    }

    /// Construct a RangeSensorLidar on the prim held by \p schemaObj .
    /// Should be preferred over RangeSensorLidar(schemaObj.GetPrim()),
    /// as it preserves SchemaBase state.
    explicit RangeSensorLidar(const UsdSchemaBase& schemaObj)
    ...
    /// Return a RangeSensorLidar holding the prim adhering to this
    /// schema at \p path on \p stage.  If no prim exists at \p path on
    /// \p stage, or if the prim at that path does not adhere to this schema,
    /// return an invalid schema object.  This is shorthand for the following:
    ///
    /// \code
    /// RangeSensorLidar(stage->GetPrimAtPath(path));
    /// \endcode
    ///
    RANGESENSORSCHEMA_API
    static RangeSensorLidar
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `float rotationRate = 20` |
    /// | C++ Type | float |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Float |
    RANGESENSORSCHEMA_API
    UsdAttribute GetRotationRateAttr() const;

    /// See GetRotationRateAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    RANGESENSORSCHEMA_API
    UsdAttribute CreateRotationRateAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `bool highLod = 1` |
    /// | C++ Type | bool |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Bool |
    RANGESENSORSCHEMA_API
    UsdAttribute GetHighLodAttr() const;

    /// See GetHighLodAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    RANGESENSORSCHEMA_API
    UsdAttribute CreateHighLodAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `float horizontalFov = 360` |
    /// | C++ Type | float |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Float |
    RANGESENSORSCHEMA_API
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `bool enableSemantics = 0` |
    /// | C++ Type | bool |
    /// | \ref Usd_Datatypes "Usd Type" | SdfValueTypeNames->Bool |
    RANGESENSORSCHEMA_API
    UsdAttribute GetEnableSemanticsAttr() const;

    /// See GetEnableSemanticsAttr(), and also 
    /// \ref Usd_Create_Or_Get_Property for when to use Get vs Create.
    /// If specified, author \p defaultValue as the attribute's default,
    /// sparsely (when it makes sense to do so) if \p writeSparsely is \c true -
    /// the default for \p writeSparsely is \c false.
    RANGESENSORSCHEMA_API
    UsdAttribute CreateEnableSemanticsAttr(VtValue const &defaultValue = VtValue(), bool writeSparsely=false) const;

...
class TestOgn(ogts.OmniGraphTestCase):
    ...
    TEST_DATA = [
        {
            '': {
                'inputs': [
                    ['inputs:renderProductPath', "/Render/OmniverseKit/HydraTextures/omni_kit_widget_viewport_ViewportTexture_0", False],
                ],
                'outputs': [
    ...
    async def test_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_isaacsim_core_nodes_IsaacReadCameraInfo", "isaacsim.core.nodes.IsaacReadCameraInfo", test_run, _TestGraphAndNode())
            await controller.evaluate(test_info.graph)
            _test_verify_scene(self, controller, test_run, test_info, f"isaacsim.core.nodes.IsaacReadCameraInfo User test case #{i+1}")
    ...
    async def test_vectorized_generated(self):
        for i, test_run in enumerate(self.TEST_DATA):
            ...
            controller = og.Controller()
            await _test_clear_scene(self, test_run, reset_stage=False)
            test_info = await _test_setup_scene(self, controller, f"/TestGraph{i}", "TestNode_isaacsim_core_nodes_IsaacReadCameraInfo","isaacsim.core.nodes.IsaacReadCameraInfo", test_run, _TestGraphAndNode(), 16)
            await controller.evaluate(test_info.graph)
            _test_verify_scene(self, controller, test_run, test_info, f"isaacsim.core.nodes.IsaacReadCameraInfo User test case #{i+1}", 16)
    ...
    async def test_data_access(self):
        ...
        test_node = og.Controller.node("/TestGraph/Template_isaacsim_core_nodes_IsaacReadCameraInfo")
